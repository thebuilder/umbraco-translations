...
const facet = (overrides: Partial<LocaleFacet>): LocaleFacet => ({
  code: "da", name: "Danish", isDefault: false, isConfigured: true,
...
describe("editingMode", () => {
  it("is the ordinary one for a language the application ships text in", () => {
    expect(editingMode(facet({ messageCount: 1284, overriddenCount: 118, absentKeyCount: 0 }))).toBe("search");
  });
  ...
  it("stays a queue once some of it has been written", () => {
    ...
    expect(editingMode(facet({ messageCount: 412, overriddenCount: 412, absentKeyCount: 872 }))).toBe("queue");
  });
  ...
  it("is not a queue once nothing is left absent, whatever else is true of it", () => {
    ...
    expect(editingMode(facet({ messageCount: 1284, overriddenCount: 1284, absentKeyCount: 0 }))).toBe("search");
  });
...
describe("coverageOf", () => {
  it("measures what has been written against every key there is", () => {
    expect(coverageOf(facet({ overriddenCount: 412 }), 1284))
      .toEqual({ written: 412, total: 1284, fraction: 412 / 1284 });
  });
