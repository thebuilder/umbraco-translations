...
export const api = {
  ...
  permissions: (signal?: AbortSignal) =>
    unwrap(TranslationsService.permissionsGetPermissions({ ...requestOptions, signal })),
  sources: (signal?: AbortSignal) =>
    unwrap(TranslationsService.sourcesListSources({ ...requestOptions, signal })),
  source: (id: string) =>
    unwrap(TranslationsService.sourcesGetSource({ ...requestOptions, path: { id } })),
  createSource: (source: SourceRequest) =>
    unwrap(TranslationsService.sourcesCreateSource({ ...requestOptions, body: source })),
  updateSource: (id: string, source: SourceRequest) =>
    unwrap(
      TranslationsService.sourcesUpdateSource({ ...requestOptions, body: source, path: { id } })
    ),
  deleteSource: (id: string) =>
    unwrap(TranslationsService.sourcesDeleteSource({ ...requestOptions, path: { id } })),
  testSource: (id: string) =>
    unwrap(TranslationsService.sourcesTestSource({ ...requestOptions, path: { id } })),
  testSourceConfiguration: (source: SourceRequest) =>
    unwrap(TranslationsService.sourcesTestSourceConfiguration({ ...requestOptions, body: source })),
  syncSource: (id: string) =>
    unwrap(TranslationsService.sourcesSyncSource({ ...requestOptions, path: { id } })),
  syncHistory: (id: string) =>
    unwrap(TranslationsService.sourcesListSourceSyncs({ ...requestOptions, path: { id } })),
  messages: (filters: {
    locale?: string;
    namespace?: string;
    query?: string;
  ...
  message: (id: string, signal?: AbortSignal) =>
    unwrap(TranslationsService.messagesGetMessage({ ...requestOptions, path: { id }, signal })),
  ...
  messageKeyIds: (
    query: Omit<MessageKeyQuery, "compare" | "sort" | "direction" | "page" | "pageSize">,
    signal?: AbortSignal
  ) =>
