...
import { type DeliveryFormat, type DeliveryMode, deliveryEndpoint } from "./app/source-form.js";

@customElement("thebuilder-translations-output-api")
class TranslationOutputApiElement extends UmbElementMixin(LitElement) {
  ...
  async #copy(url: string): Promise<void> {
    try {
      ...
      this.#notification?.peek("positive", {
        data: {
          headline: "Endpoint copied",
          message: "The translation output URL is on your clipboard.",
        },
      });
    } catch {
      this.#notification?.peek("danger", {
        data: { headline: "Could not copy endpoint", message: "Select and copy the URL manually." },
      });
    }
  }
  ...
  render() {
    ...
    const outputUrl = selected
      ...
      : undefined;
    const localeOptions = [...new Set(this.endpoints.map((endpoint) => endpoint.locale))].map(
      (locale) => ({ name: locale, value: locale, selected: locale === this._selectedLocale })
    );
    ...
    ].map((messageNamespace) => ({
      name: messageNamespace,
      value: messageNamespace,
      selected: messageNamespace === this._selectedNamespace,
    }));
    const formatOptions = this.endpoints
      .filter(
        (endpoint) =>
          endpoint.locale === this._selectedLocale && endpoint.namespace === this._selectedNamespace
      )
      .map((endpoint) => ({
        name: deliveryFormatLabel(endpoint.format),
        value: endpoint.format,
        selected: endpoint.format === this._selectedFormat,
      }));
    ...
    return html`
      <uui-box headline="Translation output API" headline-variant="h2">
        ...
        ${
          // Mutually exclusive with the line above: no endpoints means no selection, and no
          // selection means no URL to build.
          outputUrl
            ? html`
            <div class="builder">
              <uui-form-layout-item><uui-label slot="label" for="output-content">Contents</uui-label><uui-select id="output-content" label="Output contents" .options=${contentOptions} @change=${(
                event: Event
              ) => {
                const value = String((event.currentTarget as UUISelectElement).value);
                if (value === "overrides" || value === "all") {
            ...
            <p class="summary">${
              this._deliveryMode === "overrides"
                ? "Only the editorial overrides. Merge these over the defaults your app already ships."
                : "Every synced message, with editorial overrides applied."
            }</p>
            <div class="result">
              <code>${outputUrl}</code>
              <div class="actions">
                <uui-button type="button" look="secondary" label="Copy translation output URL" @click=${() => {
                  this.#copy(outputUrl);
                }}>Copy URL</uui-button>
